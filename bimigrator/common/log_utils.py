"""
Simplified logging utilities for BIMigrator.
...
3. Filters out unnecessary logs for generated PBIT files
...
ENV_LOG_LEVEL = 'BIMIGRATOR_LOG_LEVEL'
ENV_LOG_OUTPUT_FILES = 'BIMIGRATOR_LOG_OUTPUT_FILES'
...
def configure_logging(workbook_name: Optional[str] = None) -> None:
    ...
    # Add file handler if workbook name is provided
    if workbook_name:
        # Create logs directory if it doesn't exist
        logs_dir = Path('logs')
        logs_dir.mkdir(exist_ok=True)
        
        # Create a log file with workbook name and timestamp
        from datetime import datetime
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        clean_name = ''.join(c if c.isalnum() else '_' for c in workbook_name)
        log_file = logs_dir / f'bimigrator_{clean_name}_{timestamp}.log'
...
def log_file_operation(file_path: str, operation_type: str, details: Optional[str] = None) -> None:
    """Log a file operation, filtering out PBIT file operations unless explicitly enabled.
    ...
    # Check if this is an extracted file (e.g., calculations.json)
